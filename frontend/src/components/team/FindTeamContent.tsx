...
export interface FindTeamContentProps {
  teams: TeamCardViewModel[];
  people: PeopleCardViewModel[];
  isLoading: boolean;
}

/**
 * Presentational component for the Find Team page content.
 * Used by both the real find-team page and the onboarding flow.
 * Contains data-tour attributes on key elements for coachmark targeting.
 */
export default function FindTeamContent({ teams, people, isLoading }: FindTeamContentProps) {
  ...
  return (
    <div className="space-y-8 w-full">
      ...
      <div className="flex justify-center w-full">
        <div data-tour="tab-toggle" className="flex bg-[#EAEAEA] rounded-xl p-1 shrink-0 w-full max-w-[280px]">
          <button
          ...
          filteredTeams.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full items-start">
              {filteredTeams.map((team, idx) => (
                <div key={team.id} data-tour={idx === 0 ? "first-card" : undefined}>
                  <TeamCard data={{
                    id: team.id,
